...
from loss.LMMD import LMMDLoss as LMMD
...
def plot_confusion_matrix(True_label, T_predict1, batch_idx):
    ...
    ax.set_xlabel('predict(1,2->0)')
    ax.set_ylabel('true(1,2->0)')
    # bottom, top = ax.get_ylim()
    # ax.set_ylim(bottom + 0.5, top - 0.5)

    # ax.set_ylim(len(C1)-0.5, -0.5)
    plt.savefig('C:/Users/12-0/PU120{}.svg'.format(batch_idx))
    # plt.show()
